    ...
    friendElements.forEach(function (friend) {
        ...
        if (player.NumberOfVACBans || player.NumberOfGameBans) {
            ...
            if (player.NumberOfGameBans) {
                text += player.NumberOfGameBans + ' Бан от патруля' + (player.NumberOfGameBans > 1 ? 's' : '');
            }

            if (player.NumberOfVACBans) {
                text += (text === '' ? '' : ', ') + player.NumberOfVACBans + ' VAC бан' + (player.NumberOfVACBans > 1 ? 's' : '');
            }
            text += ' ' + player.DaysSinceLastBan + ' дней' + (player.DaysSinceLastBan > 1 ? '' : '') + ' спустя.';

            span.style.color = 'rgb(255, 73, 73)';
            span.textContent = text;
        } else if (greentext) {
            span.style.color = 'rgb(43, 203, 64)';
            span.textContent = 'Банов нет.';
        }
